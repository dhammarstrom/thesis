...
Before the training intervention, participants reported to the laboratory for bilateral baseline muscle biopsies (*m. vastus lateralis*).
...
Muscle thickness (*m. vastus lateralis*) was measured bilaterally before the study and two and eight days after the last training session in the experimental group.
...
## Participnats included in RNAseq
participants_rna <- read.csv("./data/study-1b/data/oneThreeSetLeg.csv", sep=";")
...
studyIb <- participants_rna %>%
  inner_join(characteristics) %>%
  filter(subject!="FP10") %>% # removes participant excluded prior to comecing the study
  filter(timepoint=="pre") %>%
  inner_join(cybex.temp) %>%
  dplyr::select(subject, sex, rnaseq_include, age, height, weight, fat.whole, lean.whole,BMC.whole, trq)%>%
  mutate(fat=((fat.whole/1000)/weight)*100,
         trq.lean=trq/(lean.whole/1000)) %>%
  inner_join(participants) %>%
        mutate(group = if_else(rnaseq_include == "incl", "RNA-seq", "Excluded"), 
               sex = factor(sex, levels = c("female", "male"), 
                            labels = c("Female", "Male")), 
               study = "Study I", 
               bmi = weight/(height/100)^2) %>%
  filter(group == "RNA-seq") %>%
        dplyr::select(participant = subject, 
                      study,
                      sex,group, age, height, weight,bmi,
                      fat.whole, 
...
rbind(studyI, studyIb, studyII) %>%
        ...
        mutate(m.sd = if_else(variable == "age", paste0(sprintf("%.1f", m), " (", sprintf("%.1f", sd), ")"), 
               ...
               group = factor(group, levels = c("Included", "Excluded","RNA-seq", "experiment", "control"), 
                              labels = c("Included", "Excluded","RNA-seq", "Training", "Control"))) %>%
  ...
  footnote(general = "Data are means and (SD). A subset of participants recruited for Study I were included in RNA-sequencing experiments referred to as RNA-seq.",
           general_title = "", 
...
In Study II, protein was extracted from Trizol preparations following suggestions by Kopec *et al.* [@RN2050]<!-- Kopec trizol protocol -->
...
Total-protein content, defined as the mean grey value of the whole well, was quantified with ImageJ [@RN2259]. Quantification of between-well gray values was subtracted as background. Image Studio Lite (LI-COR Biotechnology, Lincoln, Nebraska USA) was used to quantify chemiluminescence signals. Primary antibodies used in both studies are presented in Table \@ref(tab:primary-antibodies-table).

```{r primary-antibodies-table, echo = FALSE, message = FALSE, warning = FALSE, results = "asis"}

data.frame(type = c(rep("Primary", 8),
                    rep("Secondary", 2)),
                    target = c("mTOR\\textsuperscript{Ser2448}", 
                      "pan-mTOR", 
                      "p85 S6K1\\textsuperscript{Thr412}", 
                      "p70 S6K1\\textsuperscript{Thr389}", 
                      "pan-S6K1", 
                      "rpS6\\textsuperscript{Ser235/236}", 
                      "pan-rpS6", 
                      "pan-UBF", 
                      "Anti-mouse IgG", 
                      "Anti-rabbit IgG"), 
...
Threshold cycles (Ct) were estimated from the models by the second-derivate maximum method with technical duplicates modeled independently [@RN1768].
...
[@RN1964], with slight modifications. In Study I, log-transformed data were fitted to mixed linear models using the `lme` function in the `nlme` package
[@RN1986].
...
[@RN1964].
Here qPCR-data was transformed to counts [@RN1964] and modeled in Poisson generalized mixed linear models using the `MCMCglmm` package
[@RN1992].
...
## Meta-analysis of resistance training volume-dependent effects on muscle mass and strength
...
meta-analyses [@RN1767; @RN789]. For more recent studies, PubMed, Google Scholar, and SportDiscuss searches were made with search terms being "training volume", "resistance training", "strength training", "set",
...
Participant characteristics were coded based on sex (male, female or mixed when a study failed to discern between males and females), age (young, \< 30 years; middle-aged, 30-50 years; old, \> 50 years; or mixed), training status (trained, \> 1 session per week during the last six months leading up to the intervention; untrained \< 1 session). Study groups were considered independent also in studies utilizing within-participant models.
...
From `r n_studies` studies, a total of `r total_es[1, 2]` and `r total_es[2, 2]` effect sizes were coded for muscle mass and strength measurements, respectively. Mixed-effects meta-regression models were used to model the effect of the weekly number of sets on training-induced muscle mass and strength gains.
...
Data analyses were predominantly performed in R [@rtats].
...
For each paper included in this thesis, data and code are hosted on github (Paper I, [github.com/dhammarstrom/benefits-of-rt-volume](https://github.com/dhammarstrom/benefits-of-rt-volume); Paper II, [github.com/dhammarstrom/rnaseq-pipeline](https://github.com/dhammarstrom/rnaseq-pipeline);
Paper III, [github.com/dhammarstrom/ribo-accum-paper](https://github.com/dhammarstrom/ribo-accum-paper);
