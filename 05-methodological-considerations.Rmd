...
Sigma <- matrix(co, nrow=2, ncol=2) + diag(2)
 
rawvars <- mvrnorm(n=30, mu=mu, Sigma=Sigma, empirical = TRUE)
?mvrnorm

cor(rawvars[,1], rawvars[,2])

t.test(rawvars[,1], rawvars[,2], paired =)
...
set.seed(1)
ran_var <- rnorm(100, 0, 4)

results <- data.frame(correlation = rep(NA, 1000), 
                      est_lm = rep(NA, 1000), 
                      se_lm = rep(NA, 1000), 
                      tval_lm = rep(NA, 1000), 
                      est_lmm = rep(NA, 1000), 
                      se_lmm = rep(NA, 1000), 
                      tval_lmm = rep(NA, 1000))


for(i in 1:100) {
  
  temp <- mr %>%
         mutate(change_mod = if_else(sets == "multiple", 
                              change + sample(ran_var, 34, replace = FALSE), 
                              change)) 
  
  results[i, 1] <- temp %>% 
  dplyr::select(subject, sets, change_mod) %>%
  pivot_wider(names_from = sets, values_from = change_mod) %>%
  with(., cor(single, multiple))

  
  lm_mod <- temp %>%
    lm(change_mod ~ sets, data = .)
  ...
  results[i, 2] <- coef(lm_mod)[2] 
  results[i, 3] <- coef(summary(lm_mod))[2,2] 
  results[i, 4] <- coef(summary(lm_mod))[2,2] 
  results[i, 5] <- coef(summary(lme_mod))[2,1] 
  results[i, 6] <- coef(summary(lme_mod))[2,2] 
  results[i, 7] <- coef(summary(lme_mod))[2,4] 
...
within <- mr %>%
  dplyr::select(subject, sets, change) %>%
  pivot_wider(names_from = sets, values_from = change) %>%
  mutate(diff.sq = (multiple - single)^2) %>%
  summarise(sd = sqrt(sum(diff.sq)/(2*n()))) %>%
...
mr %>%
  ...
  with(., t.test(single, multiple, paired = TRUE))
...
between <- mr %>%
  ...
  pivot_wider(names_from = sets, values_from = change) %>%
  rowwise() %>%
  mutate(avg = mean(c(multiple, single))) %>%
  ungroup() %>%
  summarise(sd = sd(avg)) %>%
  print()
...
model <- 5.41765513^2 / 2.15760867^2
...
mr %>%
  ...
  mutate(diff = multiple - single) %>%
  inner_join(temp) %>%
  ggplot(aes(setsmultiple, diff)) + geom_point() + geom_abline(intercept = 0, slope = 1) + 
...
Mapping tools were used with default settings, this  Although each mapping tool may be further optimized, these analyses shows that selection of tools with default settings  
