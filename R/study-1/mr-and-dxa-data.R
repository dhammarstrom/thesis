...
dat <- read_excel("./data/study-1/body-composition/MR.xlsx")
...
code <- read_excel("./data/study-1/body-composition/MR_codeSubject.xlsx")
...
####### Extract DXA based measures ################

dxa <- read_excel("./data/study-1/body-composition/bodycomp_DXA.xlsx") %>%
        dplyr::select(subject, timepoint, lean.right_leg, lean.left_leg) %>%
        pivot_longer(names_to = "leg", values_to = "g", cols = lean.right_leg:lean.left_leg) %>%
        mutate(leg = if_else(leg == "lean.right_leg", "R", "L")) 
