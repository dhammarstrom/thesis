...
\hypertarget{within-session-training-volume-affects}{%
\section{Within-session training volume affects}\label{within-session-training-volume-affects}}
...
additional studies have been published investigating the effect of training volume on muscle mass \emph{and} muscle strength. Based on studies included in the meta-analysis perfomed by Schoenfeld \emph{et al.} {[}25{]} additional studies were coded based on a systematic literature search among newly published studies (from January 2015 to June 2020). Compared to e.g.~Schoenfeld \emph{et al.} {[}25{]}, an additional 10 studies were included in the analyses.
...
Together these observations indicates larger perturbations along the mTORC1 signaling axis which confirms previous observations showing exercise-volume dependency of mTORC1 related signaling in the acute phase (\textless{} 5 hours) after exercise with the low volume condition doing at least three sets activationg the muscle under study
{[}250, 22, 21{]}.
...
Interestingly, albeit with limited resolution, Terzis \emph{et al.} and Ahtiainen \emph{et al.} did not report any clear volume dependency in exercise-induced activation of ERK 1/2
{[}168, 22{]},
...
In Study I, acute activation of S6K1 in response to the fifth session did not correlate with training-induced changes in muscle mass (measured with MRI).\footnote{Raw Spearman's correlations between exercise induced change in S6K1 phosphorylation and change in muscle mass indicated weak (insignificant) associations (\(\rho=\) -0.044 - 0.190) depedning on the isoform measured (p70 or p85) and number of sets. More elaborate modeling confirmed weak relationships between degree of phsophorylation and muscle mass change. When controlling for baseline values in degree of phsophorylation (as this influence the calculation of fold-change) and number of sets gave standardized estimates (increase in \%-muscle mass change for every 1SD change in fold-change phsophorylation, with {[}95\% CI{]}) of \(\beta=\) 0.33 {[}-0.53, 1.20{]} for p70 S6K1-p70\textsuperscript{Thr389} and \(\beta=\) 0.43 {[}-0.39, 1.25{]} for S6K1-p85\textsuperscript{Thr412}.}
