...
\hypertarget{myonuclear-addition-and-transcriptional-regulation-of-training-induced-muscle-remodeling}{%
\subsection{Myonuclear addition and transcriptional regulation of training-induced muscle remodeling}\label{myonuclear-addition-and-transcriptional-regulation-of-training-induced-muscle-remodeling}}

Skeletal muscle fibers are polynuclear cells. Each fiber contains multiple nuclei which supports its transcriptional requirements. Muscle fiber-specific nuclei (myonuclei) lack the ability to proliferate and are therefore relying on dormant cell populations located in muscle tissue to activate and supply the fiber with transcriptional capacity
{[}219{]}.
Among these cells, satellite cells are the primary source of additional myonuclei in adult skeletal muscle, however other cell types may also contribute
{[}219, 220{]}.
Satellite cells can activate and fuse with the muscle fiber upon specific stimuli such as hormonal
{[}221{]}
mechanical stress
{[}222{]}.
...
In the context of muscle hypertrophy, the role of satellite cells are somewhat unclear.
Kadi \emph{et al.} reported that although the number of satellite cells increased in response to training, the addition of myonuclei did not,
suggesting that the existing myonuclei were sufficient to provide transcriptional capacity in hypertrophied muscle
{[}227{]}.
Similarly, other studies have reported increases in muscle fiber cross-sectional area, without or minimal concomitant enlargement of the myonuclei pool
{[}228, 229{]}.
...
{[}230{]}.
However, this analysis may have been confunded by the fact that the clustering did not account for age or sex. For example, the extreme-responder group contained 9 young (20-35 years) but only 2 elderly (60-75 years) male participants whereas the non-responder group contained 1 young man and 6 elderly
{[}231{]}.
This is of importance as satellite cell responses to exercise are different between young and elderly
{[}223{]}.
A similar clustering-approach as performed in {[}230{]} did subsequently not find any difference in myonuclear addition between different response clusters in a homogeneous sample in terms of age (and sex)
{[}198{]}.
A further argument against the idea of a determining role of myonuclear addition in resistance-training induced hypertrophy comes from studies on depletion of satellite cells in mice where muscle growth induced by synergist ablation is unaffected
{[}220{]}.
Myonuclei are thus able to maintain transcriptional requirements in, possibly relating to a large degree of reserve capacity
{[}232{]}.

In addition to resistance exercise, satellite cells activate in response to a wide range of mechanical stimuli including endurance training, not leading to hypertrophy
{[}233{]}
suggesting roles in skeletal muscle remodeling other than by maintaining a transcriptional capacity in growing fibers.
Indeed, satellite cells have been shown to have an important role indirect to muscle growth but vital to tissue remodeling
{[}234{]}
...
Transcriptional control is pivotal for the maintenance of skeletal muscle and the study of gene expression have and will be a vital tool to further map the biology of muscle.

Previous studies have investigated transcriptome responses to acute resistance exercise {[}237, 119, 238{]}
and chronic resistance training {[}239, 240, 119, 241, 237, 120{]},
as well as described associations between transcriptome characteristics and degrees of muscle growth {[}120, 242{]},
and function {[}243, 244{]}.

Whereas these studies have merited interesting findings, they lack clear coherences in terms of differential expression events, even for classical exercise-inducible genes such as PGC1\(\alpha\) {[}245{]}.
...
{[}246{]} are likely to reduce this variation and to provide transcriptome data with increased biological meaningfulness. Second, technical variability can be attributed to decisions made during the bioinformatical treatment of data.
As described by Concea \emph{et al.}, there is no optimal pipeline for sequencing technology as new tools keep evolving and emerging, different tools should be explored to an optimum pipeline for the specific type of data {[}247{]}.
To exploit the potential of any study design, there is a need for identifying an appropriate pipeline for transcriptome analyses to ensure a biologically valid interpretation of data. This entails identifying potential violations of common assumptions caused by the experimental model at hand, relating to, for example, data normalization {[}248, 249{]}.

For transcriptome data to provide adequate biological information about a given experimental set-up, numerous bioinformatic steps need to be adopted in a customized manner {[}247, 250{]}.
Of these steps, data normalization is particularly decisive {[}249{]},
as it aims to transform naïve transcript counts into biologically meaningful results. This essentially means expressing them as \emph{per}-cell abundances {[}251{]}.
For most experimental models, this is equivalent to providing transcript-to-total RNA ratios, given the fulfillment of the assumption that total RNA levels remain stable between conditions on a per-unit-cell or per-unit-tissue basis {[}251{]}.
In cell models that exhibit high degrees of plasticity, gene expression events result in increased amounts of total RNA and mRNA transcripts per cell {[}252{]}, specifically violating the assumption that most genes are not differentially expressed {[}251, 248{]}.

We are not aware of any study that has addressed the need to account for such perspectives during transcriptome analyses of skeletal muscle subjected to mechanical stress, such as resistance training. Indeed, this assumption can be expected to be violated, as total RNA content increases markedly on a per-unit-weight basis {[}253{]}, with potential global changes also occurring for the mRNA pool, though this remains unknown. The extent to which total RNA, and therefore ribosomal RNA, increases, coincides with the increase in muscle mass {[}253,195{]}, underlining its importance for cellular growth but also its inevitable presence as a potential confounding factor in RNA sequencing experiments.
...
(\textgreater{} 3 sets, \textless{} 5 hours; {[}20,146{]} vs. {[}21,22,255{]}).
...
described in {[}263{]} as the biopsy needle itself could be used to
...
well tolerated {[}263,265{]}. Any reported differences in fiber type
distributions between sampling techniques have been suggested relating
to differences in sampling depth {[}265,266{]}.
...
Melt-curve analyses were performed for all reactions to verify single-product amplification. Gene-specific primers were designed for all targets using Primer-BLAST {[}274{]} and Primer3Plus {[}275{]} and ordered from Thermo Scientific, except for the external RNA control, for which primers were supplied with the kit.
Raw fluorescence data was exported from the platform specific software and amplification curves were modelled with a best-fit sigmoidal model using the qpcR-package {[}276{]} written for R {[}\textbf{R-base?}{]}. Threshold cycles (Ct) were estimated from the models by the second-derivate maximum method with technical duplicates modelled independently.
Amplification efficiencies were estimated for every reaction {[}as described by 277,implemented in 276{]}. For every primer pair, mean amplification efficiencies (\(E\)) were utilised to transform data to the linear scale using \(E^{-Ct}\). Primer sequences and primer characteristics (i.e.~average primer efficiencies and Ct values) are presented in Table 2. Gene expression data was log-transformed prior to statistical analysis. As Ct-values, but not efficiencies are related to RNA integrity {[}278{]}, RQI scores were used in the statistical treatment of qPCR data to control for potential degradation effects on a by target basis (see below).
...
In Study II, an initial sample size calculation was made based on a best case scenario. Data from Brook et al. {[}279{]} indicated that a within-participant differences in RNA per tissue weight of \(15\%\) could be detected with seven participants in the experimental group (e.g.~2.79 (0.65) to 3.21 (0.74) \SI{}{\micro\gram\per\micro\litre}, \(\alpha<0.05\), \(\beta<0.2\), effect size=1.2). The control group was included in the study as a negative control primarily for the sake of systems validation of experimental procedures regarding measures of e.g.~ribosomal biogenesis. With a balanced design, accounting for drop-outs eight participants were required in each group. After a preliminary analysis of the experiment (experimental group \emph{n = }7, control group \emph{n = }7){[}280{]}, additional participants (experimental group \emph{n =} 4, control group \emph{n = }1) were recruited to the study to increase the precision of estimates, primarily in analyses within the experimental group.
...
\bottomrule
\multicolumn{4}{l}{\textsuperscript{a} Estimates from Wernbom et al. {[}282{]}}\\
...
\leavevmode\hypertarget{ref-RN804}{}%
\CSLLeftMargin{{[}233{]} }
\CSLRightInline{Joanisse S, Gillen JB, Bellamy LM, McKay BR, Tarnopolsky MA, Gibala MJ, et al. Evidence for the contribution of muscle stem cells to nonhypertrophic skeletal muscle remodeling in humans. FASEB J 2013. \url{https://doi.org/10.1096/fj.13-229799}.}

\leavevmode\hypertarget{ref-RN1880}{}%
\CSLLeftMargin{{[}234{]} }
\CSLRightInline{Baker MJ, Frazier AE, Gulbis JM, Ryan MT. Mitochondrial protein-import machinery: Correlating structure with function. Trends Cell Biol 2007;17:456--64. \url{https://doi.org/10.1016/j.tcb.2007.07.010}.}
