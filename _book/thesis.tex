...
A possible growth limiting role for satellite cells in the context of muscle hypertrophy is to maintain a fixed (minimum) number of myonuclei per muscle fiber volume through myonuclear accretion, in order to maintain transcriptional capacity
{[}219{]}.
...
{[}198{]}.
A further argument against the idea of a determining role of myonuclear addition in resistance-training induced hypertrophy comes from studies on mice where initial muscle growth induced by synergist ablation is unaffected despite of specific depletion of satellite cells
{[}220{]}.
...
A change in transcription in response to mechanical loading is not only quantitative but also qualitative with multiple transcriptional programs enabling coordinated adaptation to the specific stimuli.
...
Previous studies Using large-scale techniques to investigate transcriptome characteristics in response to resistance-exercise or training, studies have evaluated {[}234, 119, 237{]}
and chronic resistance training {[}238, 239, 119, 240, 234, 120{]},
as well as described associations between transcriptome characteristics and degrees of muscle growth {[}120, 241{]},
and function {[}242, 243{]}.

From a methodological point of view, skeletal muscle subjected to e.g.~resistance training, concomitant with muscle hypertrophy thus both exhibit large global scale changes to the RNA pool as well as important \emph{qualitative} changes to RNA sub-populations.
...
{[}233, 246, 195, 197, 193{]}.
Depending on the technique used for evaluating RNA expression (e.g.~qPCR or RNA-seq), data normalization during analysis aims to make experimental conditions comparable.

In cell culture experiments, the common denominator for transcripis abundances {[}244{]}.

To exploit the potential of any study design, there is a need for identifying an appropriate pipeline for transcriptome analyses to ensure a biologically valid interpretation of data. This entails identifying potential violations of common assumptions caused by the experimental model at hand, relating to, for example, data normalization {[}245, 247{]}.

For transcriptome data to provide adequate biological information about a given experimental set-up, numerous bioinformatic steps need to be adopted in a customized manner {[}248, 249{]}.

{[}\textbf{RN?}{]}
cell models that exhibit high degrees of plasticity, gene expression events result in increased amounts of total RNA and mRNA transcripts per cell

specifically violating the assumption that most genes are not differentially expressed
...
Whereas these studies have merited interesting findings, they lack clear coherences in terms of differential expression events, even for classical exercise-inducible genes such as PGC1\(\alpha\) {[}251{]}.
...
{[}252{]} are likely to reduce this variation and to provide transcriptome data with increased biological meaningfulness. Second, technical variability can be attributed to decisions made during the bioinformatical treatment of data.
As described by Concea \emph{et al.}, there is no optimal pipeline for sequencing technology as new tools keep evolving and emerging, different tools should be explored to an optimum pipeline for the specific type of data {[}248{]}.
...
\begin{verbatim}
# A tibble: 1 x 4
   cor.r cor.r.p    cor.p cor.p.p
   <dbl>   <dbl>    <dbl>   <dbl>
1 0.0866   0.493 -0.00245   0.985
\end{verbatim}
\begin{verbatim}
Linear mixed-effects model fit by REML
 Data: p70_csa 
       AIC      BIC    logLik
  338.1026 350.8654 -163.0513

Random effects:
 Formula: ~1 | subject
        (Intercept) Residual
StdDev:    3.241679 1.285193
...
Variance function:
 ...
 Parameter estimates:
     expon 
0.08930536 
Fixed effects: change ~ scale(fc) + sets 
                Value Std.Error DF  t-value p-value
(Intercept)  3.528982 0.6462628 32 5.460598  0.0000
scale(fc)    0.281937 0.4218329 30 0.668362  0.5090
setsmultiple 1.390571 0.4374563 30 3.178766  0.0034
 Correlation: 
             (Intr) scl(f)
scale(fc)    -0.014       
setsmultiple -0.306  0.082

Standardized Within-Group Residuals:
        Min          Q1         Med          Q3         Max 
-1.63102414 -0.44800057  0.04070562  0.55884999  1.90469870 
